...
public class ConfigurationContext implements WebMvcConfigurer {
    ...
    Dictionary allWordsDictionary(String path) throws IOException {
        Resource resource = applicationContext.getResource("classpath:" + path);
        return new DictionaryFileStorage(resource.getURI());
    }

    @Bean
    @Value("${hidden.words.path}")
    Dictionary hiddenWordsDictionary(String path) throws IOException {
        Resource resource = applicationContext.getResource("classpath:" + path);
        return new DictionaryFileStorage(resource.getURI());
    }
